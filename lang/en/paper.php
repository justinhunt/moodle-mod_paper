...
$string['scanalignmentpadnote'] = 'Corrections apply to display-only areas. An offset can move the image by up to the crop margin, currently {$a}mm, since that is how much extra was captured around each area; asking for more than that leaves a blank strip along the edge. Submissions processed before the margin was introduced have no room to shift and are left as they are.';
$string['scanalignmentunits'] = 'Offsets are in millimetres on the printed A4 page, so they can be compared directly with the crop margin below and with a ruler held against a print-out. Most scans need a shift of one or two millimetres. The scales are separate: they are percentages, where 100 means the scan is the same size as the template, and they are only needed when the misalignment grows across the page instead of staying the same.';
